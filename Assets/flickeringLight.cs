        ...
        switch( flickeringLightStyle )
        {
            ...
            case flickerinLightStyles.CampFire:
                ...
                if( campfireMethod == campfireMethods.Range || campfireMethod == campfireMethods.Both )
                {
                    ...
                    {
                        // Cycle the Campfire angle
                        CampfireSineCycleRange += CampfireSineCycleRangeSpeed * Time.deltaTime;
                        if( CampfireSineCycleRange > 360.0f ) CampfireSineCycleRange = 0.0f;

                        // Base + Values
                        GetComponent<Light>().range = CampfireRangeBaseValue + ( ( Mathf.Sin( CampfireSineCycleRange * Mathf.Deg2Rad ) * (CampfireRangeFlickerValue / 2.0f ) ) + (CampfireRangeFlickerValue / 2.0f ) );
                    }
